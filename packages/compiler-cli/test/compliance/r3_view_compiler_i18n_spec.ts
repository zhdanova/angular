...
import {i18nIcuMsg, i18nMsg, i18nMsgWithPostprocess} from './i18n_helpers';
import {compileFnFactory, expectEmit} from './mock_compile';
...
for (const compilationModel of ['aot', 'prelink'] as const) {
  const compile = compileFnFactory(compilationModel);
  const verify = (input: string, output: string, extra: any = {}): void => {
    ...
    const opts = (i18nUseExternalIds: boolean) => ({
      i18nUseExternalIds,
      compilationModel,
      ...(extra.compilerOptions || {enableI18nLegacyMessageIdFormat: false})
    });
  ...
  describe(`[compilationModel: ${compilationModel}]`, () => {
    fdescribe('i18n support in the template compiler', () => {
      ...
      `;
          ...
          const i18n_3 = i18nMsg(
              ...
              {meaning: 'm2', desc: 'd2'});
          const i18n_4 =
              i18nMsg('{$interpolation}', [['interpolation', String.raw`\uFFFD0\uFFFD`]]);
      ...
      `;
          ...
          const i18n_2 = i18nMsg(
              'Span title {$interpolation}', [['interpolation', String.raw`\uFFFD0\uFFFD`]]);
          const i18n_3 = i18nMsg(
      ...
      `;
          ...
          const exceptions = {
            '3052001905251380936':
                'Wrapper message generated by "ng xi18n" around ICU: "  {$ICU}  "'
          };
          verify(input, output, {exceptions});
        });
      ...
      fdescribe('line ending normalization', () => {
        [true, false].forEach(
...
def\`;
        …
        $I18N_4$ = $localize \`:${ICU_EXPRESSION_ID}␟4863953183043480207:{VAR_SELECT, select, =0 {zero
  }}\`
        …
...
$` + String.raw`{$I18N_4$}:ICU:\`;
      ...
      describe('es5 support', () => {
        if (compilationModel === 'prelink') {
          // The prelink model does not produce ES5 code
          return;
        }
      ...
      `;
          ...
          verify(input, output, {
            skipIdBasedCheck: true,
            compilerOptions: {target: ts.ScriptTarget.ES5, enableI18nLegacyMessageIdFormat: false}
          });
        });
      });
